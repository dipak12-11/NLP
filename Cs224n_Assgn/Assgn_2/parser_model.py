...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(self.hidden_size, self.n_features * self.embed_size))
    ...
    def forward(self, w):
        ...
        h=torch.matmul(x,self.embed_to_hidden_weight.t())+self.embed_to_hidden_bias
        h_relu=F.relu(h)
        h_drop=self.dropout(h_relu)
        logits=torch.matmul(h_drop,self.hidden_to_logits_weight.t())+self.hidden_to_logits_bias        
